...
import io.github.phantamanta44.botah2.util.SafeJsonWrapper;
...
public class Deck {
	...
	public static Deck parseCardcast(String name, JsonObject cards) {
		SafeJsonWrapper data = new SafeJsonWrapper(cards);
		Deck deck = new Deck(name);
		deck.addWhites(StreamSupport.stream(data.getJsonArray("responses").spliterator(), true)
				.map(e -> StringEscapeUtils.unescapeHtml4(e.getAsJsonObject().get("text").getAsJsonArray().get(0).getAsString()))
				.collect(Collectors.toList()));
		deck.addBlacks(StreamSupport.stream(data.getJsonArray("calls").spliterator(), true)
				.map(e -> {
					JsonObject card = e.getAsJsonObject();
                    String text = StreamSupport.stream(card.get("text").getAsJsonArray().spliterator(), false)
                            .map(o -> StringEscapeUtils.escapeHtml4(o.getAsString()))
                            .reduce((a, b) -> a.concat(" ___ ").concat(b)).orElse("");
					return new BlackCard(text, card.get("text").getAsJsonArray().size() - 1);
				})
				.collect(Collectors.toList()));
		return deck;
	}
